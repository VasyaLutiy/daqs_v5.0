(define (domain social-unified-v4)
  ...
  (:constants
    id_act_blade_inspect - action-id
    id_act_dagger_twirl - action-id
    id_act_armor_adjust - action-id
    light - tag
    small - tag
    magical - tag
    armor - tag
  ...
  (:action do_act_dagger_twirl
      :parameters (?a - agent ?item_h - item ?tag_h - tag
      )
      :precondition (and 
          (current-mood ?a cynical)
          (holding ?a ?item_h)
          (has-tag ?item_h ?tag_h)
          (is-tag ?tag_h dagger)
      )
      :effect (and 
          (visual-event-triggered id_act_dagger_twirl)
      )
  )
  (:action do_act_blade_inspect
      :parameters (?a - agent ?item_h - item ?tag_h - tag
      )
      :precondition (and 
          (current-mood ?a serious)
          (holding ?a ?item_h)
          (has-tag ?item_h ?tag_h)
          (is-tag ?tag_h blade)
      )
      :effect (and 
          (visual-event-triggered id_act_blade_inspect)
      )
  )
  (:action do_act_armor_adjust
      ...
      :effect (and 
          (visual-event-triggered id_act_armor_adjust)
      )
  )
)